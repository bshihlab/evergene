...
make_survival_plot <- function(cancer_full_name, 
	...
	surv_cox_results <- tryCatch({
		...
		res<-c(gene, gene_full_name, cancer_full_name, beta, HR, wald.test, p.value)
		names(res) <- c("gene", "gene_full_name", "cancer", "beta", "HR (95% CI for HR)", "wald.test", "p.value")
		res
	}, error = function(e) {
	...
	stats_result_vec["p.value"] <-  surv_results
...
#	par(mar = c(2, 2, 9, 2))
	...
	titlePlot <-
		paste0(
			  ...
			  "Cox regression [ p.val=", surv_cox_results["p.value"], ", beta=", surv_cox_results["beta"],",\n    HR(95%CI)=" , surv_cox_results["HR (95% CI for HR)"], " ]")
#	plot(
#	  fit,
#		col = plot_cols,
#		xlab = "Days",
